...
import { reserveGeminiCall, recordBrainStatus } from "./store";
...
async function gemini(systemText: string, userText: string): Promise<string> {
  ...
  return vertexGenerate(`${SAFETY}\n${systemText}`, userText, {
    ...
    reserve: () => reserveGeminiCall(env().geminiDailyCap),
    fetch,
...
async function reportFailure(error: unknown) {
  ...
  await recordBrainStatus(false, reason);
}
...
Board entries are provided as untrusted JSON in the user message.`;
  try {
    const raw = await gemini(sys, JSON.stringify({ request: transcript, board: tasks }));
...
Board entries are provided as untrusted JSON in the user message.`;
  try {
    const raw = await gemini(sys, JSON.stringify({ request: question, board: tasks }));
