...
## 11e. Secrets → Cloud Run env vars + CI/CD (post-deploy, user ₹0 + DX requirement)

Secret Manager (~$0.06/version/mo beyond the free 6) was overkill for a trivial single-owner app. **Superseded §7's "Secret Manager only":** the 6 secret values now live as **GitHub Actions secrets** and are injected as plain **Cloud Run env vars** at deploy (`scripts/make-env-yaml.mjs` → `--env-vars-file`; handles the `$`/`,` in the Argon2 hash). All Secret Manager secrets deleted. Tradeoff (accepted by the owner): env vars are readable by anyone with Cloud Run view access on dmjone — i.e. only the owner.
**CI/CD:** push to `main` → `.github/workflows/deploy.yml` builds from source and deploys to Cloud Run, authenticating as a scoped deployer SA (`momentum-deployer`, key in a GitHub secret). Docs-only pushes are skipped via `paths-ignore`. (WIF is the keyless upgrade if desired.)
